...
app.post("/compra", async (req, res, c, d, s, f, m, cu, ca, ce, cv) => {
  const {nombre, direccion, ciudad, departamento, sexo, fecha, metfact, cardnum, cardnam, cardexp, cardcvv} = req.body;
  let total = 0;

  const carrito = req.session.carrito || [];
  carrito.forEach(p => total += p.precio);
  const idCliente = await clienteModel.insertar(nombre, direccion, ciudad, departamento, sexo, fecha, metfact, cardnum, cardnam, cardexp, cardcvv);
  const idVenta = await ventaModel.insertar(idCliente, total, "PENDIENTE");
